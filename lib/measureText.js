...
function getCacheKey(text, width, fontFace, fontSize, lineHeight, maxLines) {
    return text + width + fontFace.id + fontSize + lineHeight + maxLines;
}
 ...
 */
module.exports = function measureText(text, width, fontFace, fontSize, lineHeight, maxLines) {
    var cacheKey = getCacheKey(text, width, fontFace, fontSize, lineHeight, maxLines);
    ...
    if (measuredSize.width <= width) {
        ...
        while (bk = breaker.nextBreak() && lineIndex < maxLines) {
